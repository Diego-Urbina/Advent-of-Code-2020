...
Day14::Day14(std::string inputFile) : mInputFile{inputFile}
{
    utils::format::print_title(14);
    ReadData();
}

long long Day14::Puzzle1()
{
    ExecProgram1();

    const long long result =
      std::accumulate(std::begin(mMemory), std::end(mMemory), 0ll,
...
long long Day14::Puzzle2()
{
    ...
    utils::format::print_answer(2, result);
    return result;
}
