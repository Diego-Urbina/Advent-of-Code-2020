...
int Day11::Puzzle1()
{
    ...
    int answer = CountOccupiedSeats(mSeatsTarget);
...
bool Day11::ProcessSeatWithAdjacents(int row, int col, unique_ptr<vector<string>> &source,
                                 ...
                                 const std::unique_ptr<std::vector<std::string>> &source)
{
    ...
    for (int r = row - 1; r <= row + 1; ++r) {
        for (int c = col - 1; c <= col + 1; ++c) {
            ...
            if (r >= 0 && r < source->size() && c >= 0 && c < (*source)[r].size()) {
                if ((*source)[r][c] == '#') {
                    ++occupied;
                }
            }
