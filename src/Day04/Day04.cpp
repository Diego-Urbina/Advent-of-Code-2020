...
int Day04::Puzzle1()
{
    auto answer = std::count_if(mPassports.begin(), mPassports.end(),
                                [](const unique_ptr<Passport> &p) { return p->IsValid(false); });
...
{
    auto answer = std::count_if(mPassports.begin(), mPassports.end(),
                                [](const unique_ptr<Passport> &p) { return p->IsValid(true); });
...
{
    if (mFields.size() == 8 || (mFields.size() == 7 && mFields.count("cid") == 0)) {
        return !validateFields || BirthYearValid() && IssueYearValid() && ExpirationYearValid() &&
...
bool Passport::EyeColorValid()
{
    ...
    return ec == "amb" || ec == "blu" || ec == "brn" || ec == "gry" || ec == "grn" || ec == "hzl" ||
           ec == "oth";
}
